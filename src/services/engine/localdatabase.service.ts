...
export class LocalDatabaseService {
  ...
  async updateUserAnswer(id: number, answer: Answers): Promise<void> {
    await this.db.put('userChart', { ...answer, id });
  }
  ...
  async getUserByChartAndTask(chartId: number, taskId: number): Promise<Answers[]> {
    const all = await this.getUserAnswers();

    return all.filter((x) => x.chart_id === chartId && x.task_id === taskId);
  }
  async validateUserLine(userLine: Answers): Promise<boolean> {
    const adminLines = await this.getByChartAndTask(userLine.chart_id!, userLine.task_id!);
   ...
   */
  async validateLinesAgainstDb(chartId: number, taskId: number): Promise<Map<number, boolean>> {
