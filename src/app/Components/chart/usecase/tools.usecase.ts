...
export class ToolsUsecase {
  ...
  /** Aborts an in-progress line draw: resets drawing flags, unlocks the chart, and removes the temporary preview series. */
  ...
  /** Pops the most recent undo snapshot and restores lines from it, clearing selection/handles and re-rendering. No-op (with a toast) if the undo stack is empty. */
  public async undoLastChange(): Promise<void> {
    ...
    for (const line of snapshot.newDrawLine) {
      const saved = await this.localdb.createUserAnswer({
    ...
    jsonToCandle.clearHandlesCanvas;
  ...
  public canDraw(): boolean {
    return this.chartstate.remainingLines > 0;
  }
}