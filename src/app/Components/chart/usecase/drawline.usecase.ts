...
export class DrawingUsecase {
  ...
  public handleChartClick(param: any): void {
    ...
    if (!this.chartState.isDrawing) {
      ...
      // removed lockChartInteraction() — it blocks the second click
  ...
  /** Converts a chart-space (time, price) point into pixel-space screen coordinates. Returns null if either axis can't resolve the value (e.g. out of visible range). */
  ...
  /** Converts a pixel-space point on the chart canvas into chart-space (time, price). Returns null for negative coordinates or if lightweight-charts can't resolve the point. */
 ...
 private async finishDrawing(endPoint: Point): Promise<void> {
    ...
    // Save to local DB first — and store the id back on the line so later
    // drag/extend calls UPDATE this row instead of inserting a new one.
    const savedRecord = await this.localdb.createAnswer(this.toLineRecord(newLines, false, false));
   ...
   private toLineRecord(
      ...
      return {
        answer_id: line.answer_id ?? null, // ← preserve server answer_id across edits (was hardcoded null)
        chart_id: this.chartState.chartId,
  ...
  public updatePreviewLine(param: any): void {
      ...
      try {
        ...
        if (this.chartState.activeTool === 'hline' || this.chartState.activeTool === 'ray') {
          // hline/ray need to be extended to the edges of the visible range.
          ...
          const t1 = this.chartState.drawingStartPoint!.time,
            t2 = end.time;
          if (t1 === t2) return;
          // lightweight-charts requires series data sorted ascending by time.
          const ordered =
            t1 < t2
              ? [
      ...
      /** Computes the (time, value) endpoints of a line extended to the full visible time range, using the line's slope/intercept — used for hline/ray previews. */
