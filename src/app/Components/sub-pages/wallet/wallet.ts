...
import { WalletCreatation, WalletCreatationRes, WalletTransactionRequest } from './models/wallet.model';
...
private toDateInputValue(d: string | Date): string {
  ...
  if (this.depositTxnDate > this.maxDate) {
    ...
    this.usecase.walletaction(payload).pipe(
...
console.log(typeof this.withdrawTxnAmount);
 ...
 createWallet(): void {
  ...
  this.submitError = '';
  this.loader.show();
  this.usecase.execute(payload).pipe(finalize(() => this.loader.hide())).subscribe({
    next: (res) => {
  ...
  loadWallet(): void {
    ...
    this.usecase.getWallet()
      ...
      console.log('Wallet ID:', this.wallet?.create_date);
            this.currentBalance = res.data.amount;
           this.cdr.detectChanges()
